...
class Auth extends CI_Controller
{
	...
	{
		// pengecekan agar tidak bisa kembali ke auth
		...
		if ($user) {
			...
			if ($user['is_active'] == 1) {
				...
				if (password_verify($password, $user['password'])) {
					...
					if ($user['role_id'] == 1) {	//jika role id nya 1 masuk ke admin
						redirect('admin');
					} else {					//jika bukan maka redirect ke user
						redirect('user');
					}
				} else {
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'NuryanaNnN10@gmail.com',
			'smtp_pass' => 'nuryana12345',
			'smtp_port' => 465,
			'mailtype' => 'html',
			'charset' => 'utf-8',
			'newline' => "\r\n"
		...
		$this->email->from('NuryanaNnN10@gmail.com', 'Nuryana N');
